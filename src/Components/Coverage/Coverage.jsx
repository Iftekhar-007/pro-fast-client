...
const FlyToLocation = ({ position }) => {
  const map = useMap();
  map.flyTo(position, 11); // Zoom in closer
  return null;
};
...
const Coverage = () => {
  ...
  const handleSearch = (e) => {
    ...
    const filtered = warehouses.filter(
      (wh) =>
        normalize(wh.city).includes(query) ||
        normalize(wh.district).includes(query) ||
        normalize(wh.region).includes(query) ||
        wh.covered_area.some((area) => normalize(area).includes(query))
    );
    setSearchResults(filtered.slice(0, 5)); // Top 5 matches
  };
  ...
  const mapCenter = [23.8, 90];

  return (
    <section className="w-9/12 mx-auto h-[95vh] p-4 lg:mb-32">
      <h1 className="text-3xl font-bold text-center mb-4 text-primary">
        Delivery Coverage Map
      </h1>

      {/* 🔍 Search Box */}
      <div className="w-full relative max-w-xl mx-auto mb-4">
        <input
          type="text"
          placeholder="Search city, district, area..."
          className=" input input-bordered w-full z-[1000]"
          value={searchText}
          onChange={handleSearch}
        />
        {/* 💡 Autocomplete dropdown */}
        {searchResults.length > 0 && (
          <ul className="absolute  mt-1 w-full bg-white border rounded shadow-md max-h-60 overflow-auto z-[1001]">
            {searchResults.map((wh, i) => (
              <li
                key={i}
                className="p-2 hover:bg-gray-100 cursor-pointer z-1"
                onClick={() => handleSelectLocation(wh)}
              >
                <strong>{wh.city}</strong> — {wh.district}, {wh.region}
              </li>
            ))}
          </ul>
        )}
